...
class Rates(BaseModel):
    pregame: float = Field(..., ge=2, description="Sets pre-game rotation rate. (Min=2.0)")
    live: float = Field(..., ge=2, description="Sets live-game rotation rate. (Min=2.0)")
    final: float = Field(..., ge=2, description="Sets post-game rotation rate. (Min=2.0)")


class Rotation(BaseModel):
    enabled: bool = Field(..., description="Enable rotation of games.")
    only_preferred: bool = Field(..., description="Rotate only preferred teams.")
    rates: Rates
    while_preferred_team_live: bool = Field(..., description="Rotate games while preferred team is live.")
